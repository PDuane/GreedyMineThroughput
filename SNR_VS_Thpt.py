...
# robot_loc = (22, 357)
rx_loc = Node((117,452,1), 0, float("inf"))
robot_loc = [floor(rx_loc.x / scale), floor(rx_loc.y / scale)]
height = 1
num_nodes = 4

c_map_snr = np.zeros(np.shape(mine))
c_map_tpt = np.zeros(np.shape(mine))

for x in range(0,len(mine)):
    for y in range(0,len(mine[x])):
        if (mine[x][y] == 1):
            best_thpt = 0
            loss = rt.raytrace_loss(((x+0.5)*scale, (y+0.5)*scale, height), (rx_loc.x, rx_loc.y, rx_loc.h), 2.4e9, env)
            if (loss <= 0):
                thpt = 0
            else:
                loss = 10*log10(loss)
...
snr_diff = np.max(c_map_snr) - np.min(c_map_snr)
snr_norm = (c_map_snr + np.min(c_map_snr)) / snr_diff
# snr_norm = c_map_snr / np.max(c_map_snr)
snr_inv = np.multiply(1 - c_map_snr, obs_mine)

tpt_norm = c_map_tpt / np.max(c_map_tpt)
tpt_inv = np.multiply(1 - c_map_tpt, obs_mine)

image_snr = np.dstack((snr_inv, snr_norm, np.zeros(np.shape(snr_norm))))
image_tpt = np.dstack((tpt_inv, tpt_norm, np.zeros(np.shape(tpt_norm))))
image_map = np.repeat(mine[:,:,np.newaxis], 3, axis=2)

image_snr[floor(rx_loc.x / scale),floor(rx_loc.y / scale),0] = 0
image_snr[floor(rx_loc.x / scale),floor(rx_loc.y / scale),1] = 0
image_snr[floor(rx_loc.x / scale),floor(rx_loc.y / scale),2] = 1

image_tpt[floor(rx_loc.x / scale),floor(rx_loc.y / scale),0] = 0
image_tpt[floor(rx_loc.x / scale),floor(rx_loc.y / scale),1] = 0
image_tpt[floor(rx_loc.x / scale),floor(rx_loc.y / scale),2] = 1

image_map[floor(rx_loc.x / scale),floor(rx_loc.y / scale),0] = 0
image_map[floor(rx_loc.x / scale),floor(rx_loc.y / scale),1] = 0
image_map[floor(rx_loc.x / scale),floor(rx_loc.y / scale),2] = 1
...
img_map_gen = Image.fromarray(np.uint8(image_map.swapaxes(0,1)))
img_map_gen.show()
img_map_gen.save("C:\\Users\\Worman 212\\Documents\\PDuane_NIOSH\\NodeDeploymentFullAlgo\\GreedyMineThroughput\\Map_cmp.png")

img_snr_gen = Image.fromarray(np.uint8(image_snr.swapaxes(0,1)))
img_snr_gen.show()
img_snr_gen.save("C:\\Users\\Worman 212\\Documents\\PDuane_NIOSH\\NodeDeploymentFullAlgo\\GreedyMineThroughput\\SNR_cmp.png")

img_tpt_gen = Image.fromarray(np.uint8(image_tpt.swapaxes(0,1)))
img_tpt_gen.show()
img_tpt_gen.save("C:\\Users\\Worman 212\\Documents\\PDuane_NIOSH\\NodeDeploymentFullAlgo\\GreedyMineThroughput\\Throughput_cmp.png")
...
#             new_node, cmap = predict_node(mine, scale, dropped_nodes, 3, noise, 350*10**6, env)
#             path, length = pathfind(mine, robot_loc, [floor(new_node.x / scale), (new_node.y / scale)])
