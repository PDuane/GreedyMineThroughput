...
def calc_raytrace(tx:tuple, map:Array, omap:Array, map_size:tuple, scale, mine:me.Environment, active, lock:Lock):
    ...
    for x in range(len(env_map)):
        for y in range(len(env_map[x])):
            if env_map[x][y] == 1:
                x1_val = int((tx[0]+0.5) / scale)
                y1_val = int((tx[1]+0.5) / scale)
                x2_val = int((x+0.5) / scale)
                y2_val = int((y+0.5) / scale)
                snr = rtloss.raytrace_loss((x1_val, y1_val,tx[2]), (x2_val,y2_val,tx[2]), 2.4e9, mine)
                total += snr
    lock.acquire()
    otpt_map[tx[0], tx[1]] = total
    lock.release()
    active.value -= 1
        ...
        for y1 in range(len(phys_map[x1])):
            if phys_map[x1][y1] == 1:
                ...
                while (active_processes.value > (num_processes - 1)):
                    pass
                p = Process(target=calc_raytrace, args=((x1, y1, 2), mp_phys_map, mp_cmap, (len(phys_map), len(phys_map[0])), scale, mine, active_processes, mutex))
